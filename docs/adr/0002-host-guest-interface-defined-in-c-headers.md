...
The interface between the Host firmware and Guest Programs is defined using C headers annotated with C23 `[[clang::...]]` attributes for Host/Guest Linkage. `rudelblinken-bindgen` parses these headers and generates guest bindings (C headers and Rust).
