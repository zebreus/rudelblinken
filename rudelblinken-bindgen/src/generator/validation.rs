...
///
/// This wrapper is the internal seam between C-shaped parser IR and generator IR.
/// Validation proves the restricted canonical C subset is meaningful for
/// rudelblinken-bindgen before lowering constructs backend-facing declarations.
...
impl ValidatedDeclarations {
    pub(crate) fn validate(declarations: parser::Declarations) -> Result<Self, Vec<LoweringError>> {
        let mut validator = Validator::default();
        validator.validate(&declarations)?;
        Ok(Self { declarations })
    }

    pub(crate) fn into_inner(self) -> parser::Declarations {
        self.declarations
    }
}
...
pub(super) fn is_void_parameter_list(parameters: &[parser::Parameter]) -> bool {
    matches!(
        parameters,
        [parser::Parameter {
            name: None,
