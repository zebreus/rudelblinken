...
`Span` is a plain struct (`source: String, start: usize, end: usize`) defined
in the `parser` module. It implements `ariadne::Span` with `SourceId = String`
so it can be passed directly to ariadne without conversion. Using a custom type
rather than `chumsky::span::SimpleSpan` prevents chumsky from leaking into the
public API surface.
...
`LoweringError` gains `span: Option<parser::Span>`.  Every error pushed in
...
### ariadne rendering belongs to the CLI

`render_errors(errors, source_text) -> String` is a public helper in `lib.rs`
that formats errors with ariadne.  `generate_bindings` itself does not call it
— it returns `Vec<BindgenError>` as before.  Only `run()` (the CLI entry point)
calls `render_errors`.  Programmatic callers keep full access to the structured
errors and are not forced into ariadne's text format.
